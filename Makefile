...
HTTPS_GIT := github.com/ielboulo/ielboulo.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=ielboulo \
		  -X github.com/cosmos/cosmos-sdk/version.AppName=ielboulod \
...
ifeq ($(OS),Windows_NT)
	...
	go build -mod=readonly $(BUILD_FLAGS) -o build/ielboulod ./cmd/ielboulod
endif

build-windows-client: go.sum
	GOOS=windows GOARCH=amd64 go build -mod=readonly $(BUILD_FLAGS) -o build/ielboulod.exe ./cmd/ielboulod
...
install: go.sum
	go install -mod=readonly $(BUILD_FLAGS) ./cmd/ielboulod
...
draw-deps:
	...
	@goviz -i ./cmd/ielboulod -d 2 | dot -Tpng -o dependency-graph.png
...
ifeq (,$(shell which heighliner))
	...
	heighliner build -c ielboulo --local -f chains.yaml
endif
...
set-testnet-configs:
	ielboulod config set client chain-id localchain-1
	ielboulod config set client keyring-backend test
	ielboulod config set client output text

# import keys from testnet.json into test keyring
setup-testnet-keys:
	-`echo "decorate bright ozone fork gallery riot bus exhaust worth way bone indoor calm squirrel merry zero scheme cotton until shop any excess stage laundry" | ielboulod keys add acc0 --recover`
	-`echo "wealth flavor believe regret funny network recall kiss grape useless pepper cram hint member few certain unveil rather brick bargain curious require crowd raise" | ielboulod keys add acc1 --recover`
