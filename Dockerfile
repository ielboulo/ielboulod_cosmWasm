...
# then log output of file /code/bin/ielboulod
# then ensure static linking
RUN LEDGER_ENABLED=false BUILD_TAGS=muslc LINK_STATICALLY=true make build \
  && file /code/build/ielboulod \
  && echo "Ensuring binary is statically linked ..." \
  && (file /code/build/ielboulod | grep "statically linked")

# --------------------------------------------------------
FROM alpine:3.16

COPY --from=go-builder /code/build/ielboulod /usr/bin/ielboulod
